...
;(function($, window, document, underfined) {
    ...
    function getTimeRemaining() {
        // Get stored end time from localStorage or set a new one (30 days from now)
        let endTime = localStorage.getItem('countdownEndTime');
    
        if (!endTime) {
            endTime = new Date();
            endTime.setDate(endTime.getDate() + 30); // Set countdown for 30 days
    ...
    function updateClock($clock) {
        ...
        if (t.total <= 0) {
            ...
            localStorage.removeItem('countdownEndTime'); // Reset after completion
        }
    }
    ...
    if ($('.prague-coming-time-wrapper').length) {
        ...
        comingTimeWrapper.each(function () {
            ...
            updateClock($mask_clock); // Initial update
    
            prague_timeinterval = setInterval(function () {
                updateClock($mask_clock);
            }, 1000);
        });
    }
